...
const useMathRacing = () => {
  ...
  const generateQuestion = useCallback(async () => {
      ...
      switch(gameState.operationType) {
        ...
        default:
          const operations = [
            ...
            { text: `${Math.max(num1, num2)} - ${Math.min(num1, num2)}?`, correct: Math.max(num1, num2) - Math.min(num1, num2) }
          ];
    ...
    const audioBlob = gameState.soundOn 
      ? await generateQuestionAudio(text, process.env.REACT_APP_LMNT_API_KEY) 
      : null;
  ...
  const handleAnswer = useCallback((selectedAnswer) => {
    if (selectedAnswer === gameState.currentQuestion.correct) {
      ...
      const newPlayerPosition = Math.min(gameState.playerPosition + 150, 1350);
  ...
  const advanceOpponent = useCallback(() => {
    if (!gameState.isGameOver) {
      const newOpponentPosition = gameState.opponentPosition + gameState.difficulty;
...
const MathRacingGame = () => {
  ...
  const renderMenuScreen = () => (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-100">
      ...
      <button
        onClick={() => setGameState(prev => ({ ...prev, gameScreen: 'difficulty' }))}
  ...
  const renderGameScreen = () => {
    ...
    return (
      <div className="relative h-screen bg-gray-200">
        ...
        {/* Race Track */}
        <div className="absolute bottom-40 w-full h-40 bg-gray-300"></div>
        {/* Cars */}
        <div
        ...
        {isGameOver && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center">
            ...
            <div className="flex space-x-4">
              ...
              <button
                onClick={() => setGameState(prev => ({ ...prev, gameScreen: 'menu' }))}
  ...
  const renderCurrentScreen = () => {
    switch(gameState.gameScreen) {
      ...
      case 'operation': return renderOperationScreen();
      case 'digits': return renderDigitsScreen();
